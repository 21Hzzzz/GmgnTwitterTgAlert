...
class SQLiteStorage:
    ...
    async def start(self) -> None:
        ...
        await self._migrate_summary_runs_schema()
    ...
    async def record_summary_run(
        ...
        await self._execute(
            ...
            INSERT OR REPLACE INTO summary_runs (
                ...
                status, item_count, tg_sent, content, error
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                summary_key,
    ...
    async def _migrate_summary_runs_schema(self) -> None:
        if not self._conn:
            return
        async with self._lock:
            await asyncio.to_thread(self._migrate_summary_runs_schema_sync)
    ...
    def _migrate_summary_runs_schema_sync(self) -> None:
        table_rows = self._conn.execute("PRAGMA table_info(summary_runs)").fetchall()
        columns = {row["name"] for row in table_rows}
        ...
        if not has_old_unique and "feishu_sent" not in columns:
            return

        logger.info("🗄️ 迁移 summary_runs 为 Telegram-only 摘要结构")
        source_platform_expr = "source_platform" if "source_platform" in columns else "'telegram'"
        source_target_expr = "source_target_id" if "source_target_id" in columns else "''"
        tg_sent_expr = "COALESCE(tg_sent, 0)" if "tg_sent" in columns else "0"
        self._conn.executescript(
            ...
            INSERT OR REPLACE INTO summary_runs_new (
                ...
                tg_sent, content, error
            )
            SELECT
                id, summary_key, {source_platform_expr}, {source_target_expr},
                window_start, window_end, generated_at, status, item_count,
                {tg_sent_expr}, content, error
            FROM summary_runs;
